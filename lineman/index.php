...
if ($linemanId <= 0 && isset($_SESSION['user_id'])) {
    $possibleId = (int)$_SESSION['user_id'];
    $checkLineman = mysqli_query($conn, "SELECT id FROM linemen WHERE id = {$possibleId} LIMIT 1");
    if ($checkLineman && mysqli_num_rows($checkLineman) > 0) {
        $linemanId = $possibleId;
    }
}
...
if (!$previewAllowed && $sessionRole !== 'lineman') {
    echo 'Access denied';
    exit;
}

if ($linemanId <= 0) {
    echo 'Lineman not found in session. Please set $_SESSION["lineman_id"].';
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['collect_payment'])) {
    ...
    $checkOrderSql = "
        SELECT o.id, o.order_number, o.total_amount, o.paid_amount, o.pending_amount, o.payment_status,
               c.id AS customer_id, c.current_balance
        FROM orders o
    ...
    if (!$orderRow) {
        ...
        try {
            ...
            $sqlCustomer = "UPDATE customers SET current_balance = current_balance - ? WHERE id = ?";
...
$where = " WHERE c.assigned_lineman_id = {$linemanId} ";
...
if ($invoiceTypeFilter !== '') {
    $safeType = mysqli_real_escape_string($conn, $invoiceTypeFilter);
    $orderSql = "SELECT * FROM ({$orderSql}) t WHERE t.invoice_type = '{$safeType}' ORDER BY t.order_date DESC, t.id DESC";
} else {
...
<style>
.page-topbar{margin-left:250px;position:sticky;top:0;z-index:1000}.vertical-menu{top:0}.main-content{margin-left:250px}.footer{padding:20px 0;color:#666}.badge.bg-info-subtle,.badge.bg-secondary-subtle,.badge.bg-warning-subtle,.badge.bg-success-subtle{font-size:12px;}@media (max-width:991.98px){.page-topbar,.main-content{margin-left:0}.vertical-menu{display:none}}
</style>
<body data-sidebar="dark">
<?php include('../includes/pre-loader.php'); ?>
<div id="layout-wrapper">
    <header class="page-topbar bg-white border-bottom">
        <div class="container-fluid py-3 d-flex justify-content-between align-items-center">
            <div>
                <h5 class="mb-0">Dashboard | Welcome, <?php echo e($_SESSION['name'] ?? $lineman['full_name']); ?></h5>
                <small class="text-muted">Lineman Work Dashboard</small>
            </div>
            <div class="text-end">
                <div class="fw-semibold"><?php echo e($_SESSION['name'] ?? $lineman['full_name']); ?></div>
                <small class="text-muted">Lineman User</small>
            </div>
        </div>
    </header>
    ...
    <div class="main-content">
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body d-flex flex-wrap justify-content-between align-items-center gap-3">
                                <div>
                                    <h4 class="mb-1">Lineman Work Dashboard</h4>
                                    <div class="text-muted">
                                        <?php echo e($lineman['full_name']); ?> | <?php echo e($lineman['employee_id']); ?> | <?php echo e($lineman['assigned_area']); ?>
                                        <?php if (!empty($lineman['zone_name'])): ?> | Zone: <?php echo e($lineman['zone_name']); ?><?php endif; ?>
                                    </div>
                                </div>
                                <div class="text-end">
                                    <div class="fw-semibold">Phone: <?php echo e($lineman['phone']); ?></div>
                                    <div class="text-muted">Status: <?php echo e(ucwords(str_replace('_', ' ', $lineman['status']))); ?></div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <form method="get" class="row g-3 align-items-end">
                            ...
                            <div class="col-md-3">
                                ...
                                <select class="form-select" name="invoice_type">
                                    ...
                                    <option value="First Sale Invoice" <?php echo $invoiceTypeFilter === 'First Sale Invoice' ? 'selected' : ''; ?>>First Sale Invoice</option>
                                    <option value="Performance Invoice" <?php echo $invoiceTypeFilter === 'Performance Invoice' ? 'selected' : ''; ?>>Performance Invoice</option>
                                    <option value="Completed Invoice" <?php echo $invoiceTypeFilter === 'Completed Invoice' ? 'selected' : ''; ?>>Completed Invoice</option>
                                </select>
                            </div>
                            <div class="col-md-4">
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <span class="text-muted">Show order, needed product, invoice type, collection</span>
                        </div>
                            ...
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <?php else: ?>
                                    ...
                                    <tr>
                                        ...
                                        <td><span class="badge bg-info-subtle text-info"><?php echo e($order['invoice_type']); ?></span></td>
                                        <td>
                                            <div><span class="badge bg-secondary-subtle text-secondary"><?php echo e(ucwords($order['status'])); ?></span></div>
                                            <div class="mt-1"><span class="badge bg-warning-subtle text-warning"><?php echo e(ucwords($order['payment_status'])); ?></span></div>
                                        </td>
                                        <td>
                                            <div class="d-flex flex-column gap-2">
                                                <a href="../view-invoice.php?id=<?php echo $orderId; ?>" class="btn btn-sm btn-primary">View Invoice</a>
        ...
        <footer class="footer"><div class="container-fluid"><div class="row"><div class="col-12">&copy; <?php echo date('Y'); ?> Ecommer - Developed by ecommer.in</div></div></div></footer>
    </div>
</div>
