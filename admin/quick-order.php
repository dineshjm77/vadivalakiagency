...
$preselected_product_quantity = isset($_GET['quantity']) ? intval($_GET['quantity']) : 1;

// Fetch all active customers (admin can see all)
$customers_sql = "SELECT c.id, c.customer_code, c.shop_name, c.customer_name, c.customer_contact,
                         c.shop_location, c.current_balance, c.payment_terms, l.full_name as lineman_name
                  FROM customers c 
...
// Check if preselected customer exists
$preselected_customer = null;
if ($preselected_customer_id > 0) {
    $check_sql = "SELECT c.*, l.full_name as lineman_name 
                  FROM customers c 
                  LEFT JOIN linemen l ON c.assigned_lineman_id = l.id 
                  WHERE c.id = $preselected_customer_id 
                  AND c.status = 'active'";
        ...
        try {
            ...
            if ($paid_amount > 0) {
                ...
                mysqli_stmt_bind_param($payment_stmt, "idsssi", $customer_id, $paid_amount, 
                                     $payment_method, $order_number, $payment_notes, $admin_id);
...
<body data-sidebar="dark">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card">
                                <div class="card-body">
                                    <h4 class="card-title mb-4">Create New Order (Admin)</h4>
                                    ...
                                    <div class="alert alert-info mb-3">
                                        ...
                                        <?php if ($preselected_customer['lineman_name']): ?>
                                        | Assigned Lineman: <?php echo $preselected_customer['lineman_name']; ?>
                                        <?php endif; ?>
                                    </div>
                                    <?php endif; ?>
                                                    ...
                                                    <div class="card-body">
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <select class="form-select" id="customer_id" name="customer_id" required>
                                                                ...
                                                                <?php 
                                                                // Reset pointer and loop through customers
                                                                mysqli_data_seek($customers_result, 0);
                                                                while ($customer = mysqli_fetch_assoc($customers_result)): 
                                                                    $selected = ($preselected_customer_id == $customer['id']) ? 'selected' : '';
                                                                ...
                                                                <option value="<?php echo $customer['id']; ?>" 
                                                                        ...
                                                                        data-terms="<?php echo $customer['payment_terms']; ?>"
                                                            ...
                                                            <div class="col-md-6">
                                                                <div class="mb-3">
                                                                    ...
                                                                    <input type="text" class="form-control" id="customer_balance" readonly
                                                                           value="<?php echo $preselected_customer ? '₹' . number_format($preselected_customer['current_balance'], 2) : '₹0.00'; ?>">
                                                                </div>
                                                            </div>
                                                            <div class="col-md-6">
                                                                <div class="mb-3">
                                                                    ...
                                                                    <input type="text" class="form-control" id="payment_terms" readonly
                                                                           value="<?php echo $preselected_customer ? ucfirst(str_replace('_', ' ', $preselected_customer['payment_terms'])) : ''; ?>">
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <textarea class="form-control" id="shop_location" rows="2" readonly><?php echo $preselected_customer ? htmlspecialchars($preselected_customer['shop_location']) : ''; ?></textarea>
                                                        </div>
                                                        
                                                        <div class="mb-3">
                                                            <label class="form-label">Assigned Lineman</label>
                                                            <input type="text" class="form-control" id="assigned_lineman" readonly
                                                                   value="<?php echo $preselected_customer ? $preselected_customer['lineman_name'] : ''; ?>">
                                                        </div>
                                                    </div>
                                                </div>
    ...
    <script>
        ...
        document.getElementById('customer_id').addEventListener('change', function() {
            ...
            const terms = selectedOption.getAttribute('data-terms') || '';
            ...
            document.getElementById('payment_terms').value = terms.replace(/_/g, ' ');
        ...
        function resetForm() {
            if (confirm('Are you sure you want to reset the form? All entered data will be lost.')) {
                ...
                if (preselectedCustomerId > 0) {
                    ...
                    document.getElementById('shop_location').value = '';
                    document.getElementById('customer_balance').value = '₹0.00';
                    document.getElementById('payment_terms').value = '';
                    document.getElementById('assigned_lineman').value = '';
                }
                ...
                // Auto-add preselected product again if exists
                if (preselectedProductId > 0) {
                    setTimeout(autoAddPreselectedProduct, 500);
                }
            }
        }

        // Load draft on page load (removed save draft for admin)
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // If we have a preselected customer, trigger change event
            if (preselectedCustomerId > 0) {
                const event = new Event('change');
                document.getElementById('customer_id').dispatchEvent(event);
            }
